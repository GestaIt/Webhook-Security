...
def insert_guild(guild_id: str, logging_channel_id: str, model_logging_channel_id: str) -> bool:
    with sqlite3.connect(games_database) as connection:
        cursor = connection.cursor()

        try:
            cursor.execute("INSERT INTO guildData VALUES (:gID, :lCI, :mCI)",
                           {"gID": guild_id, "lCI": logging_channel_id, "mCI": model_logging_channel_id})
        except sqlite3.Error:
...
def get_log_channel_id(guild_id: str, is_logging: bool) -> tuple[bool, str]:
    with sqlite3.connect(games_database) as connection:
        cursor = connection.cursor()
